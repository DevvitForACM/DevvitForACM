...
import { initializeRedditNotificationService } from './services/reddit-notification.service';
...
async function initializeServer() {
  // Initialize Firebase and check health
  await logFirebaseHealth();
  ...
  if (isDevvitContext) {
    ...
    // Initialize Reddit notification service with the Reddit client
    initializeRedditNotificationService(reddit);

    const postModule = await import('./core/post');
    createPost = postModule.createPost;
  } else {
    // Initialize Reddit notification service without client for standalone mode
    initializeRedditNotificationService();
  }
